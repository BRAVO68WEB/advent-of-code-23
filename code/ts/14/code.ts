...
const [platformX, platformY] = [
  platformOriginal[0].length,
  platformOriginal.length,
];
...
const loadOnNorth = (platform: string[][]) =>
  platform.reduce(
    (acc, v, i) =>
      acc + (platform.length - i) * v.filter((x) => x === "O").length,
    0,
  );
...
const flipWest = (platform: string[][]) =>
  transpose(flipNorth(transpose(platform)));
const flipEast = (platform: string[][]) =>
  flipWest(platform.map((x) => x.toReversed())).map((x) => x.toReversed());
const flipSouth = (platform: string[][]) =>
  flipNorth(platform.toReversed()).reverse();
const cycle = (platform: string[][]) =>
  flipEast(flipSouth(flipWest(flipNorth(platform))));
