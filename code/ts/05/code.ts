...
const toLocation = (maps: Maps, value: Seed) =>
    maps.reduce((curr, map) => {
        for (const m of map) {
            if (curr >= m.src[0] && curr < m.src[1]) return curr + m.dest[0] - m.src[0];
        }
        return curr;
    }, value);
...
const toSeed = (maps: Maps, value: Seed) =>
    maps.reduceRight((curr, map) => {
        for (const element of map) {
            const m = element;
            if (curr >= m.dest[0] && curr < m.dest[1]) return curr + m.src[0] - m.dest[0];
        }
        return curr;
    }, value);
...
const part2 = (input: string) => {
    ...
    const possibleSeeds = maps
        .flatMap((m, i) => m.map(({ dest }) => toSeed(maps.slice(0, i + 1), dest[0])))
