...
function walk(
  ...
  for (let n = 0; n < 4; n++) {
    ...
    const toX = xy[0] + nx;
    const toY = xy[1] + ny;
    if (toX < 0 || toY < 0 || toX === map.length || toY === map.length)
      continue;
...
const part2 = (input: string): string | number => {
  ...
  const innerCornerStepsLeft =
    steps - stepsToCorner - (diagonalReach - 1) * size;
  ...
  const innerNWplots = getPlots(
    map,
    [size - 1, size - 1],
    innerCornerStepsLeft,
  );
  const innerSWplots = getPlots(map, [size - 1, 0], innerCornerStepsLeft);
  const innerCornerPlots =
    innerNEplots + innerSEplots + innerNWplots + innerSWplots;
  const outerNEplots = getPlots(map, [0, size - 1], outerCornerStepsLeft);
  const outerSEplots = getPlots(map, [0, 0], outerCornerStepsLeft);
  const outerNWplots = getPlots(
    map,
    [size - 1, size - 1],
    outerCornerStepsLeft,
  );
  const outerSWplots = getPlots(map, [size - 1, 0], outerCornerStepsLeft);
  const outerCornerPlots =
    outerNEplots + outerSEplots + outerNWplots + outerSWplots;
